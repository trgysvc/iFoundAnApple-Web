...
export async function handleIyzico3DCallback(
  request: Request
): Promise<Response> {
  try {
    ...
    if (paymentError || !paymentRecord) {
      console.error(
        "[IYZICO_CALLBACK] Payment record not found:",
        paymentError
      );
      return new Response(
    ...
    if (params.paymentId) {
      ...
      if (statusResult.success) {
        ...
        if (updateError) {
          console.error(
            "[IYZICO_CALLBACK] Failed to update payment:",
            updateError
          );
        } else {
          ...
          if (escrowError) {
            console.error(
              "[IYZICO_CALLBACK] Failed to update escrow:",
              escrowError
            );
          }
          ...
          if (deviceError) {
            console.error(
              "[IYZICO_CALLBACK] Failed to update device status:",
              deviceError
            );
          }
        ...
        if (updateError) {
          console.error(
            "[IYZICO_CALLBACK] Failed to update payment status:",
            updateError
          );
        }
    ...
    console.log(
      "[IYZICO_CALLBACK] Redirecting to PaymentCallbackPage:",
      redirectUrl
    );
...
export async function iyzicoCallbackHandler(
  request: Request
): Promise<Response> {
