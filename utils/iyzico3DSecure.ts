...
export const create3DSecurePaymentRequest = (paymentData: {
  ...
  const request = {
    ...
    // Callback URL
    callbackUrl: `${
      process.env.VITE_IYZICO_CALLBACK_URL || "http://localhost:5173"
    }/api/webhooks/iyzico-3d-callback?conversationId=${
...
export const process3DSecurePayment = async (
  paymentData: any,
  cardInfo: any
): Promise<{
  success: boolean;
  ...
  return await withErrorHandling(
    async () => {
      console.log("[3D_SECURE] 3D Secure ödeme işlemi başlatılıyor...", {
        ...
        cardNumber:
          cardInfo.cardNumber.substring(0, 6) +
          "****" +
...
export const test3DSecurePayment = async (
  amount: number = 100
): Promise<any> => {
  const paymentData = {
    amount,
  ...
  return await process3DSecurePayment(
    paymentData,
    THREEDS_TEST_CARDS["3d-secure"]
  );
};
